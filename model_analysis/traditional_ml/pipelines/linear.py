...
def run(data: str, seed: int):
    ...
    NAME = f"{datetime.today().strftime('%m%d')}-{data}-logistic-bestparams-seed{seed}"
    ...
    best_params = {
        'ALL': { 'C': 51.06681412569412, 'solver': 'lbfgs' },
        'CF': { 'C': 86.34875884532995, 'solver': 'lbfgs' },
        'CON': { 'C': 0.0016530619167889038, 'solver': 'liblinear' },
        'PN': { 'C': 5.897723940255765, 'solver': 'newton-cg' },
        'RIES': { 'C': 0.0010000270807916938, 'solver': 'lbfgs' }
    }
    ...
    model = LogisticRegression(class_weight='balanced', random_state=seed, C=best_params[data]['C'], solver=best_params[data]['solver'])
