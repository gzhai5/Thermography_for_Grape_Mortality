...
def run(data: str, seed: int):
    ...
    NAME = f"{datetime.today().strftime('%m%d')}-{data}-svm-bestparams-seed{seed}"
    ...
    best_params = {
        'ALL': { 
            'C': 22.364202820542708,
            'gamma': 3.413981076557398,
            'kernel': 'linear',
            },
        'CF': { 
            'C': 25.361101250718455,
            'gamma': 4.975636992427353,
            'kernel': 'linear',
            },
        'CON': { 
            'C': 1,
        ...
        'PN': { 
            'C': 0.7507359373086417,
            'gamma': 0.07594391619085299,
            'kernel': 'rbf',
            },
        'RIES': { 
            'C': 0.01,
            'gamma': 0.10276682751913979,
            'kernel': 'linear',
            }
    }
    ...
    model = SVC(class_weight='balanced', random_state=seed, probability=True, **best_params[data])
