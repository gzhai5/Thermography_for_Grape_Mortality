...
def run(data: str, seed: int):
    # load the data
    if data == "ALL":
        X_test = np.load('../../data-preprocess/data-split/data/mean-time-series/test/X_test.npy')
        y_test = np.load('../../data-preprocess/data-split/data/features/test/y_test.npy')
        X_train = np.load('../../data-preprocess/data-split/data/mean-time-series/train/X_train.npy')
        y_train = np.load('../../data-preprocess/data-split/data/features/train/y_train.npy')
    elif data == "CF":
        X_test = np.load(r'C:\Users\Mud\Desktop\2ndPaper\lstm\cultivar-specific\data\cf\X_test_cf.npy')
        y_test = np.load(r'C:\Users\Mud\Desktop\2ndPaper\lstm\cultivar-specific\data\cf\y_test_cf.npy')
        X_train = np.load(r'C:\Users\Mud\Desktop\2ndPaper\lstm\cultivar-specific\data\cf\X_train_cf.npy')
        y_train = np.load(r'C:\Users\Mud\Desktop\2ndPaper\lstm\cultivar-specific\data\cf\y_train_cf.npy')
    elif data == "CON":
        X_test = np.load(r'C:\Users\Mud\Desktop\2ndPaper\lstm\cultivar-specific\data\conc\X_test_con.npy')
        y_test = np.load(r'C:\Users\Mud\Desktop\2ndPaper\lstm\cultivar-specific\data\conc\y_test_con.npy')
        X_train = np.load(r'C:\Users\Mud\Desktop\2ndPaper\lstm\cultivar-specific\data\conc\X_train_con.npy')
        y_train = np.load(r'C:\Users\Mud\Desktop\2ndPaper\lstm\cultivar-specific\data\conc\y_train_con.npy')
    elif data == "RIES":
        X_test = np.load(r'C:\Users\Mud\Desktop\2ndPaper\lstm\cultivar-specific\data\ries\X_test_ries.npy')
        y_test = np.load(r'C:\Users\Mud\Desktop\2ndPaper\lstm\cultivar-specific\data\ries\y_test_ries.npy')
        X_train = np.load(r'C:\Users\Mud\Desktop\2ndPaper\lstm\cultivar-specific\data\ries\X_train_ries.npy')
        y_train = np.load(r'C:\Users\Mud\Desktop\2ndPaper\lstm\cultivar-specific\data\ries\y_train_ries.npy')
    elif data == "PN":
        X_test = np.load(r'C:\Users\Mud\Desktop\2ndPaper\lstm\cultivar-specific\data\pn\X_test_pn.npy')
        y_test = np.load(r'C:\Users\Mud\Desktop\2ndPaper\lstm\cultivar-specific\data\pn\y_test_pn.npy')
        X_train = np.load(r'C:\Users\Mud\Desktop\2ndPaper\lstm\cultivar-specific\data\pn\X_train_pn.npy')
    ...
    NAME = f"{datetime.today().strftime('%m%d')}-{data}-lstmfcn-nofeatures-dropout0-seed{seed}"
    ...
    # normalize the data with mean method
    X_train = normalize_curves_mean_method(X_train)
    X_test = normalize_curves_mean_method(X_test)
    print('Class ratio:', Counter(y_train))
    ...
    X_train = X_train.reshape(-1, 1, 600)  # Convert to (samples, 1, timesteps)
    X_test = X_test.reshape(-1, 1, 600)
    ...
    final_model = generate_lstmfcn(dropout=0)
    class_weights = compute_class_weight(y_train)
    print('class_weights:', class_weights)
    reduce_lr = ReduceLROnPlateau(monitor='loss', patience=15, mode='auto', factor=1. / np.cbrt(2), cooldown=0, min_lr=1e-6, verbose=2)
    early_stopping = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=20, verbose=1, restore_best_weights=True)
    final_model.compile(optimizer=Adam(learning_rate=5e-6), loss='binary_crossentropy', metrics=['accuracy'])
    ...
    history = final_model.fit(
        ...
        callbacks=[reduce_lr],
        shuffle=True,
        epochs=200, batch_size=16, class_weight={0: class_weights[0], 1: class_weights[1]},
        verbose=1
    )
