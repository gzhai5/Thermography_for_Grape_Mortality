...
#     #print(f"Converted len {converted_len}")
...
def sample_frame_indices(clip_len, frame_sample_rate, seg_len):
    start_idx = 0
    end_idx = start_idx + clip_len * frame_sample_rate
    indices = np.linspace(start_idx, min(end_idx, seg_len - 1), num=clip_len)
...
def frames_convert_and_create_dataset_dictionary(directory):
    ...
    for p in generate_all_files(Path(directory), only_files=True):
        ...
        #file name path
        file_name= os.path.join(directory, cls, file)
        ...
        indices = sample_frame_indices(clip_len=30, frame_sample_rate=20, seg_len=container.streams.video[0].frames)
