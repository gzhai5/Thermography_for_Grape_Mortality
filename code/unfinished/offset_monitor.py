        ...
        while(CONTINUE_RECORDING):
            try:
                ...
                if image_result.IsIncomplete():
                    ...
                    CalibrationQueryR_node = PySpin.CFloatPtr(nodemap.GetNode('R'))
                    R_val = CalibrationQueryR_node.GetValue()
                    # print('R =', R)

                    CalibrationQueryB_node = PySpin.CFloatPtr(nodemap.GetNode('B'))
                    B_val = CalibrationQueryB_node.GetValue()
                    # print('B =', B)

                    CalibrationQueryF_node = PySpin.CFloatPtr(nodemap.GetNode('F'))
                    F_val = CalibrationQueryF_node.GetValue()
                    # print('F =', F)

                    CalibrationQueryX_node = PySpin.CFloatPtr(nodemap.GetNode('X'))
                    X_val = CalibrationQueryX_node.GetValue()
                    # print('X =', X)

                    CalibrationQueryA1_node = PySpin.CFloatPtr(nodemap.GetNode('alpha1'))
                    A1_val = CalibrationQueryA1_node.GetValue()
                    # print('alpha1 =', A1)

                    CalibrationQueryA2_node = PySpin.CFloatPtr(nodemap.GetNode('alpha2'))
                    A2_val = CalibrationQueryA2_node.GetValue()
                    # print('alpha2 =', A2)

                    CalibrationQueryB1_node = PySpin.CFloatPtr(nodemap.GetNode('beta1'))
                    B1_val = CalibrationQueryB1_node.GetValue()
                    # print('beta1 =', B1)

                    CalibrationQueryB2_node = PySpin.CFloatPtr(nodemap.GetNode('beta2'))
                    B2_val = CalibrationQueryB2_node.GetValue()
                    # print('beta2 =', B2)

                    CalibrationQueryJ1_node = PySpin.CFloatPtr(nodemap.GetNode('J1'))    # Gain
                    J1_val = CalibrationQueryJ1_node.GetValue()
                    # print('Gain =', J1)

                    CalibrationQueryJ0_node = PySpin.CIntegerPtr(nodemap.GetNode('J0'))   # Offset
                    J0_val = CalibrationQueryJ0_node.GetValue()
                    # print('Offset =', J0)
                    ...
                    F_vec.append(F_val)
                    X_vec.append(X_val)
                    A1_vec.append(A1_val)
                    A2_vec.append(A2_val)
                    B1_vec.append(B1_val)
                    B2_vec.append(B2_val)
                    J1_vec.append(J1_val)
                    J0_vec.append(J0_val)
                    time.sleep(1)
                    i += 1
                    passed = int(time.time()-start_time)
                    passed_str = str(passed)
                    print("finish " + str(i) + " round and time passed " + passed_str + "!!!!!!!!!!!")

                    if (passed >= duration):
                        print("strat saving data!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                        np.save('./offsets/R_Value.npy',np.array(R_vec))
                        np.save('./offsets/B_Value.npy',np.array(B_vec))
                        np.save('./offsets/F_Value.npy',np.array(F_vec))
                        np.save('./offsets/X_Value.npy',np.array(X_vec))
                        np.save('./offsets/A1_Value.npy',np.array(A1_vec))
                        np.save('./offsets/A2_Value.npy',np.array(A2_vec))
                        np.save('./offsets/B1_Value.npy',np.array(B1_vec))
                        np.save('./offsets/B2_Value.npy',np.array(B2_vec))
                        np.save('./offsets/J1_Value.npy',np.array(J0_vec))
                        np.save('./offsets/J0_Value.npy',np.array(J0_vec))
