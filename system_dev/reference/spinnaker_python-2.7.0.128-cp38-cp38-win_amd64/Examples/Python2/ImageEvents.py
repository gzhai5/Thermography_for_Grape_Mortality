...
#   from the Spinnaker class, ImageEventHandler. ImageEventHandler allows the user to
#   define any properties, parameters, and the event handler itself while ImageEvent
...
class ImageEventHandler(PySpin.ImageEventHandler):
    ...
    def __init__(self, cam):
        ...
        self._processor.SetColorProcessing(PySpin.HQ_LINEAR)
...
def configure_image_events(cam):
    """
    This function configures the example to execute image event by preparing and
    registering an image event handler.
...
def reset_image_events(cam, image_event_handler):
    ...
    try:
        ...
        #
        #  *** NOTES ***
        #  It is important to unregister all image event handlers from all cameras they are registered to.
