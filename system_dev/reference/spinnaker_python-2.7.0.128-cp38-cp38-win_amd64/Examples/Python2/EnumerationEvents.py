...
# =============================================================================
#
#   EnumerationEvents.py explores camera arrival and removal events on the interfaces.
...
#
#   This example creates a user-defined class: SystemEventHandler.
#   This child class allows the user to define properties, parameters, and the event handler itself
...
from PySpin import IsAvailable
...
class SystemEventHandler(PySpin.SystemEventHandler):
    ...
    def OnInterfaceArrival(self, interfaceID):
        """
        ...
        ***NOTES***
        ...
        # with the same interface ID. If the interface ID changes, you must get the new interface
        # object from the updated interface list in order to access this interface.
        #
        ...
        # Here we go through the new interface list to find the arrived interface and display the
        # cameras connected to the new interface.
        for iface_idx in range(interface_num):
            # Select interface
            interface = interface_list.GetByIndex(iface_idx)
            node_map = interface.GetTLNodeMap()

            interface_id_node = node_map.GetNode("InterfaceID")
            # Ensure the node is valid
            if not IsAvailable(interface_id_node) or not IsReadable(interface_id_node):
                continue

            if interfaceID != PySpin.CValuePtr(interface_id_node).ToString():
                continue
            ...
            for cam_idx in range(cam_num):
                ...
                if IsAvailable(device_serial_num) and IsReadable(device_serial_num):
                    device_serial_num_value = PySpin.CValuePtr(device_serial_num).ToString()
                    print 'Device {0} is connected to interface {1}\n'.format(device_serial_num_value, interfaceID)

            '''Create interface event'''
            self.mutex.acquire()
            try:
                interface_event_handler = InterfaceEventHandler(None, interface, interfaceID, False)
                self.interface_event_handlers.update({interfaceID: interface_event_handler})
                ...
                print 'Error registering interface event handler to {0}: {1}'.format(interfaceID, ex)
            finally:
                self.mutex.release()

    def OnInterfaceRemoval(self, interfaceID):
        """
    ...
    def register_interface_event_to_system(self):
        ...
        self.system.RegisterInterfaceEventHandler(self.interface_event_handler_on_system)
    ...
    def unregister_interface_event_to_system(self):
        ...
        if self.interface_event_handler_on_system is not None:
            self.system.UnregisterInterfaceEventHandler(self.interface_event_handler_on_system)
