...
def configure_device_events(nodemap, cam):
    ...
    try:
        ...
        #
        # *** LATER ***
        # Device events handler must be unregistered manually. This must be done prior
        # to releasing the system and while the device event handlers are still in
        # scope.
        if CHOSEN_EVENT == EventType.GENERIC:

            # Device event handlers registered generally will be triggered by any device event handlers.
...
def reset_device_events(cam, device_event_handler):
    """
    This function resets the example by unregistering the device event handler.
